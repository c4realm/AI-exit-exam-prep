...
const generateSimulatedRecommendation = (userName, userDept, score, readiness_level, subject_breakdown, weakSubjects, isErrorFallback = false) => {
  ...
  const note = isErrorFallback 
    ? `*(Note: Displaying offline study recommendations as the AI service is currently unreachable.)*`
    : `*(Note: Running in offline advisor mode. Configure your \`GEMINI_API_KEY\` in the server's \`.env\` file to enable real-time dynamic AI recommendations.)*`;
...
const generateSimulatedChatResponse = (userName, userDept, message, readinessScore, readinessLevel, recentExamsCount, subjectAverages, weakSubjects, strongSubjects, isErrorFallback = false) => {
  ...
  const note = isErrorFallback
    ? `*(Note: Running in offline advisor mode as the real-time AI service is currently unreachable.)*\n\n`
    : `*(Note: Running in offline advisor mode. Configure your \`GEMINI_API_KEY\` to enable fully open conversational AI.)*\n\n`;
